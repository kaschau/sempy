...
def generate_primes(ys,zs,domain,nframes,normalization='exact'):
    ...
    for k,u in zip(range(3),['u','v','w']):
        eddy_in_dom = np.where( ( domain.eddy_locs[:,1] < dom_ymax + domain.sigmas[:,k,1].max() )
        ...
        eddy_locs_in_dom[u] = domain.eddy_locs[eddy_in_dom]
        sigmas_in_dom[u] = domain.sigmas[eddy_in_dom]
        eps_in_dom[u] = domain.eps[eddy_in_dom]
    ...
    for i,(y,z) in enumerate(zip(ys,zs)):
        ...
        eps_on_line = dict()
        for k,u in zip(range(3),['u','v','w']):
            eddy_on_line = np.where( (np.abs( eddy_locs_in_dom[u][:,1] - y ) < sigmas_in_dom[u][:,k,1] )
                                   & (np.abs( eddy_locs_in_dom[u][:,2] - z ) < sigmas_in_dom[u][:,k,2] ) )
            eddy_locs_on_line[u] = eddy_locs_in_dom[u][eddy_on_line]
            sigmas_on_line[u] = sigmas_in_dom[u][eddy_on_line]
            eps_on_line[u] = eps_in_dom[u][eddy_on_line]
        ...
        for k,u in zip(range(3),['u','v','w']):
            ...
            for j,x in enumerate(xs):
                ...
                eddy_on_point = np.where( x_dist < sigmas_on_line[u][:,k,0] )
                if len(eddy_on_point[0]) == 0:
                    primes_no_norm[j,k] = 0.0
                else:
                    #Compute distances to all contributing points
                    x_dist = np.abs( eddy_locs_on_line[u][eddy_on_point][:,0] - x )
                    y_dist = np.abs( eddy_locs_on_line[u][eddy_on_point][:,1] - y )
                    z_dist = np.abs( eddy_locs_on_line[u][eddy_on_point][:,2] - z )

                    #Collect sigmas from all contributing points
                    x_sigma = sigmas_on_line[u][eddy_on_point][:,k,0]
                    y_sigma = sigmas_on_line[u][eddy_on_point][:,k,1]
                    z_sigma = sigmas_on_line[u][eddy_on_point][:,k,2]
                    ...
                    if normalization == 'jarrin':
                        ...
                        e = eps_on_line[u][eddy_on_point]
                        ck = np.matmul(L, e.T).T
                        ...
                        primes_no_norm[j,k] = prime[k]

                    elif normalization == 'exact':
                        ej = eps_on_line[u][eddy_on_point]
