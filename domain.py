...
class domain:
    ...
    def make_periodic(self, periodic_x=False, periodic_y=False, periodic_z=False):
        if True in [periodic_x, periodic_y, periodic_z]:
            print(
                "Making domain periodic in {}".format("x" if periodic_x else "")
        ...
        if self.sigma_interp is None:
            raise AttributeError(
                "Please set your domain's sigma_interp before making it periodic"
            )
        # Make periodic
        if periodic_x:
            keep_eddys = np.where(
                self.eddy_locs[:, 0] + np.max(self.sigmas[:, :, 0], axis=1)
                < self.x_length
            )
            keep_eddy_locs = self.eddy_locs[keep_eddys]
            keep_eps = self.eps[keep_eddys]
            periodic_eddys = np.where(
        ...
        if periodic_y:
            keep_eddys = np.where(
                self.eddy_locs[:, 1] + np.max(self.sigmas[:, :, 1], axis=1)
                < self.y_height
            )
            keep_eddy_locs = self.eddy_locs[keep_eddys]
            keep_eps = self.eps[keep_eddys]
        ...
        if periodic_z:
            keep_eddys = np.where(
                self.eddy_locs[:, 2] + np.max(self.sigmas[:, :, 2], axis=1)
                < self.z_width
            )
            keep_eddy_locs = self.eddy_locs[keep_eddys]
            keep_eps = self.eps[keep_eddys]
